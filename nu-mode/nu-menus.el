...
(defun nu-populate-switch ()
   ...
   (define-key nu-switch-map "w" 'whitespace-mode))
...
(defun nu-populate-save-map ()
 "Populate save map	.	"
  ...
  (define-key nu-save-map "g" 'recover-this-file)
  ...
  (define-key nu-save-map "k" 'kmacro-start-macro-or-insert-counter)
...
(defun nu-populate-mark-map ()
  ...
  (define-key nu-mark-map (kbd "y") 'mark-whole-buffer)
  (define-key nu-mark-map (kbd "s") 'mark-sexp)
  (define-key nu-mark-map (kbd "g") 'mark-paragraph)
  (define-key nu-mark-map (kbd "p") 'mark-page)
  (define-key nu-mark-map (kbd "d") 'mark-defun)
...
"Populate open map."
  ...
  (define-key nu-open-map "s" 'eshell)
...
"Populate goto map."
  ...
  (define-key nu-goto-map "a" 'bookmark-jump)
  (define-key nu-goto-map "b" 'nu-buffers-list)
  (define-key nu-goto-map "c" 'goto-char)
  (define-key nu-goto-map "d" 'previous-error)
  (define-key nu-goto-map "e" 'next-error)
  (define-key nu-goto-map "f" 'cd)
  (define-key nu-goto-map "h" 'ido-switch-buffer)
  ...
  (define-key nu-goto-map "s" 'org-switchb)
...
(defun nu-populate-replace ()
  ...
  (define-key nu-replace-map "-" 'sort-fields)
  ...
  (define-key nu-replace-map "f" 'transpose-frame)
